...
class VisionTransformer(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor, mask=None):
        ...
        mask = F.interpolate(mask.to(torch.float).unsqueeze(1), (8, 32)).to(torch.bool).squeeze(1)    ### changes

        x, encode_features = self.forward_features(x)
        # x = self.head(x)
        # import pdb;pdb.set_trace()
        output.append(NestedTensor(x, mask))
        return output, encode_features #x, encode_features
